...
export default function Research() {
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    // Discovered Projects (apply per-export row cap so free/trial users
    // can't snapshot the entire result set in one PDF).
    const allProjects = (result.projects as any[]) || [];
    const cappedProjects = applyExportCap(allProjects, plan, staffBypass);
    const projects = cappedProjects.rows;
    ...
    // Stamp the forensic license footer on every page (always-on, all plans).
    // Free / trialing users also get a diagonal banner as a visible deterrent;
    // paid plans get only the discreet footer line so the doc prints cleanly.
    const watermark = buildWatermarkLabel(user?.email);
    applyPdfWatermark(doc, watermark, { plan, staffBypass });
    ...
    if (cappedProjects.truncated) {
      toast({
        title: 'Report exported (truncated)',
        description: `Saved as ${filename}. Showed ${cappedProjects.rows.length} of ${cappedProjects.total} projects — upgrade for higher caps.`,
      });
    } else {
