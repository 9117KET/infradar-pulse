ALTER TABLE public.agent_config
  ADD COLUMN IF NOT EXISTS contact_cursor_created_at timestamptz,
  ADD COLUMN IF NOT EXISTS contact_cursor_project_id uuid;
...
BEGIN
  ...
  WHERE c.normalized_name = v_norm
    ...
    AND coalesce(lower(c.email), '') = coalesce(v_email, '')
  LIMIT 1
  FOR UPDATE;
...
CREATE OR REPLACE FUNCTION public.upsert_company_project_role(
  p_company_id uuid,
  p_project_id uuid DEFAULT NULL,
  p_candidate_id uuid DEFAULT NULL,
  p_project_name text DEFAULT '',
  p_role text DEFAULT 'participant',
  p_value_usd bigint DEFAULT NULL,
  p_source_url text DEFAULT NULL,
  p_source_key text DEFAULT 'contact-finder',
...
DECLARE
  v_id uuid;
  v_link_id uuid := coalesce(p_project_id, p_candidate_id, '00000000-0000-0000-0000-000000000000'::uuid);
  v_role text := coalesce(nullif(trim(p_role), ''), 'participant');
BEGIN
  IF p_company_id IS NULL OR (p_project_id IS NULL AND p_candidate_id IS NULL) THEN
    RETURN NULL;
  END IF;

  SELECT r.id INTO v_id
  FROM public.company_project_roles r
  WHERE r.company_id = p_company_id
    AND coalesce(r.project_id, r.candidate_id, '00000000-0000-0000-0000-000000000000'::uuid) = v_link_id
    AND r.role = v_role
  LIMIT 1
  FOR UPDATE;

  IF v_id IS NULL THEN
    INSERT INTO public.company_project_roles (
      company_id, project_id, candidate_id, project_name, role, value_usd,
      source_url, source_key, awarded_at
    ) VALUES (
      p_company_id, p_project_id, p_candidate_id, coalesce(nullif(trim(p_project_name), ''), 'Unnamed project'),
      v_role, p_value_usd,
      CASE WHEN p_source_url ~ '^https?://' THEN trim(p_source_url) ELSE NULL END,
      nullif(trim(coalesce(p_source_key, 'contact-finder')), ''), p_awarded_at
    )
    RETURNING id INTO v_id;
  ELSE
    UPDATE public.company_project_roles r SET
      project_name = coalesce(nullif(trim(p_project_name), ''), r.project_name),
      value_usd = coalesce(p_value_usd, r.value_usd),
      source_url = CASE WHEN p_source_url ~ '^https?://' THEN p_source_url ELSE r.source_url END,
      source_key = coalesce(nullif(trim(p_source_key), ''), r.source_key),
      awarded_at = coalesce(p_awarded_at, r.awarded_at)
    WHERE r.id = v_id;
  END IF;
...
REVOKE ALL ON FUNCTION public.upsert_company_project_role(uuid, uuid, uuid, text, text, bigint, text, text, date) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.upsert_company_project_role(uuid, uuid, uuid, text, text, bigint, text, text, date) TO service_role;