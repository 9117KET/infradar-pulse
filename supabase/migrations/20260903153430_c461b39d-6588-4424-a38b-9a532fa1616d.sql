...
BEGIN
  ...
  v_lat := v_candidate.lat;
  v_lng := v_candidate.lng;
  v_precision := v_candidate.coord_precision;
  IF v_lat IS NULL OR v_lng IS NULL THEN
    SELECT f.lat, f.lng INTO v_lat, v_lng FROM public.country_fallback_coords(v_candidate.country) f;
    v_precision := 'country';
  END IF;
  IF v_lat IS NULL OR v_lng IS NULL THEN
    RAISE EXCEPTION 'cannot auto-publish candidate without a location' USING ERRCODE = '22023';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.auto_promote_official_candidate(uuid, text, text) TO service_role, postgres;