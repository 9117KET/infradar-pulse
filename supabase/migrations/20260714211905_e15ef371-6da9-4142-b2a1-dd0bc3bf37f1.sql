-- Safe-cast helpers: return NULL when text isn't a valid enum member instead
-- of raising, so COALESCE can substitute a default. Used by
-- promote_project_candidate to tolerate candidate rows whose sector/status/
-- stage/region strings don't match the tighter project_* enums.

CREATE OR REPLACE FUNCTION public.safe_cast_project_sector(t text)
RETURNS public.project_sector LANGUAGE plpgsql IMMUTABLE AS $$
BEGIN RETURN t::public.project_sector; EXCEPTION WHEN others THEN RETURN NULL; END; $$;

CREATE OR REPLACE FUNCTION public.safe_cast_project_stage(t text)
RETURNS public.project_stage LANGUAGE plpgsql IMMUTABLE AS $$
BEGIN RETURN t::public.project_stage; EXCEPTION WHEN others THEN RETURN NULL; END; $$;

CREATE OR REPLACE FUNCTION public.safe_cast_project_status(t text)
RETURNS public.project_status LANGUAGE plpgsql IMMUTABLE AS $$
BEGIN RETURN t::public.project_status; EXCEPTION WHEN others THEN RETURN NULL; END; $$;

CREATE OR REPLACE FUNCTION public.safe_cast_project_region(t text)
RETURNS public.project_region LANGUAGE plpgsql IMMUTABLE AS $$
BEGIN RETURN t::public.project_region; EXCEPTION WHEN others THEN RETURN NULL; END; $$;

-- Patch promote_project_candidate to use the safe casts.
CREATE OR REPLACE FUNCTION public.promote_project_candidate(
  p_candidate_id uuid,
  p_reason text DEFAULT 'Approved from verification workbench'
) RETURNS jsonb
