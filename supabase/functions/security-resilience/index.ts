...
serve(async (req) => {
  ...
  try {
    ...
    if (!await isAgentEnabled(supabase, "security-resilience")) return pausedResponse("security-resilience");
    ...
    if (supabase) await failAgentTask(supabase, "security-resilience", taskId, runStartedAt, e);
