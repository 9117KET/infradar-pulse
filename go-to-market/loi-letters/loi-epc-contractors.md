# LOI Letter Template - EPC Contractors

**Who to send this to:** Business Development Directors and Managers at large EPC firms active in MENA and East Africa (L&T PT&D, Vinci, Bechtel, Fluor, Bouygues, CCC, Samsung C&T, POSCO)

**The pitch in one sentence:** Your BD team currently finds pre-tender signals manually across MDB portals - InfraRadar surfaces them 6-18 months before RFP at $199/month vs. $30k+/year for MEED.

---

## Priority Targets (Named, Reachable)

| Name | Company | Role | LinkedIn |
|---|---|---|---|
| Hanane Ait-Toudghi | L&T PT&D Africa | BD Director | [Link](https://www.linkedin.com/in/hanane-ait-toudghi-ing-38b051214) |
| [Vinci MENA BD] | Vinci Construction | BD Director, MENA | Search: "Vinci Construction business development Middle East" |
| [Bechtel EMEA BD] | Bechtel | BD Director, EMEA | Search: "Bechtel business development Africa" |
| [Fluor MENA] | Fluor Corporation | BD Director | Search: "Fluor project development MENA" |
| [CCC BD] | Consolidated Contractors | BD Manager | Search: "Consolidated Contractors business development" |

---

## LinkedIn First Message

```
Hi [Name],

I noticed [L&T / Vinci / Bechtel] has been [winning / bidding on / active in] [specific project or region - reference something specific to their company].

I'm building InfraRadar - a platform that surfaces pre-tender signals from World Bank, IFC, ADB, AfDB, EBRD, AIIB, and IDB project pipelines, typically 6-18 months before public RFP issuance. AI-scored projects with risk and confidence ratings, custom alerting by sector and region, and an on-demand research agent for deep project intelligence.

Given [Company]'s footprint in [MENA / East Africa], the [AfDB power transmission / World Bank transport / EBRD energy] pipeline might be the most relevant piece.

We're in early access - free for 12 months for initial users. Worth a quick call?

[Your name]
```

---

## Follow-Up Email

**Subject:** Pre-tender signals for [MENA / East Africa] infrastructure - InfraRadar early access

```
Hi [Name],

Following up on my LinkedIn message.

I'm building InfraRadar - an AI platform that aggregates tender pipelines from the World Bank, IFC, ADB, AfDB, EBRD, AIIB, and IDB and surfaces pre-tender signals 6-18 months before RFP issuance.

For an EPC BD team focused on [Africa / MENA], this means: early visibility on which AfDB and World Bank projects are entering procurement stage, which sectors are getting funded, and which countries have active pipelines - before competitors who wait for the MEED alert.

We currently track 7 MDB sources, 14 regions, 14 sectors. Coverage of IsDB, AIIB, and KfW is on the Q3 2026 roadmap.

Early access is free for 12 months in exchange for user feedback on the tender tracking features specifically. Would it make sense to take a look?

[Your name]
[InfraRadar URL]
```

---

## LOI Request (after the call)

```
Hi [Name],

Thanks for the time - good to hear how [specific workflow they mentioned, e.g., "you currently track AfDB tenders using the MEED daily alert and manual DIME database checking"].

One ask: we're applying to [Y Combinator / Techstars] and they want letters of intent from EPC BD professionals who've seen the product. It's a paragraph from your professional email - no legal commitment.

Something like:

"I am [Name], Business Development [Director / Manager] at [Company]. I have reviewed InfraRadar Pulse and intend to trial the platform to track pre-tender signals from the World Bank, AfDB, and ADB pipelines relevant to our [sector] projects in [region]. The early-stage MDB pipeline aggregation and alert features are directly relevant to our BD process. I look forward to participating in the early access program."

Your own words are fine. A reply to this email with that paragraph would help us a great deal.

[Your name]
```

---

## Formal LOI Document

```
LETTER OF INTENT - INFRARADAR PULSE EARLY ACCESS

Date: [Date]
From: [Name], [Role], [Company Name]
To: InfraRadar Pulse

I, [Name], [Role] at [Company Name], write to express my intent to trial InfraRadar Pulse as part of their early access program.

[Company Name] is active in infrastructure project delivery across [describe regions - e.g., MENA and Sub-Saharan Africa], with current and recent projects including [reference 1-2 public projects if comfortable]. Our business development team monitors the MDB project pipeline to identify and position for upcoming tender opportunities.

I have reviewed InfraRadar Pulse and find the following capabilities directly relevant to our BD process:
- Pre-tender signal detection from World Bank, IFC, ADB, AfDB, EBRD, AIIB, and IDB pipelines (6-18 months before public RFP)
- AI-scored project risk and confidence assessment
- Sector and regional filtering with custom alerting
- Stakeholder intelligence extraction (contracting authority contacts, co-financiers)
- Tender calendar with upcoming bid deadline tracking

My intended use case: [describe - e.g., "monitoring the AfDB and World Bank pipeline for power transmission and renewable energy projects in Africa entering the tender stage, supplementing our current manual monitoring process"].

I have no financial interest in InfraRadar Pulse. This letter represents an expression of intent to trial the platform, not a binding commitment to purchase.

Signed:
[Name]
[Title]
[Company Name]
[Email]
[Date]
```
