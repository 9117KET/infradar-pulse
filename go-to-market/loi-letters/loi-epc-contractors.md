# LOI Template — EPC Contractors

> **Audience:** Business Development Directors and Managers at large EPC firms — Bechtel, Fluor, Vinci, Bouygues, L&T (PT&D and ECC), Consolidated Contractors (CCC), Samsung C&T, POSCO, Larsen & Toubro, China Harbour, Acciona, Webuild, Ferrovial.
> **Strategic role:** Highest-intent revenue persona. Pre-tender intelligence has direct, quantifiable BD ROI. They have budget approval authority for $30K incumbent subscriptions today.
> **Updated:** May 2026.

---

## The pitch in one sentence

Your BD team currently finds pre-tender signals manually across 7 MDB portals + a $30K MEED subscription. InfraRadarAI surfaces them 6–18 months before public RFP, adds **contractor intelligence on your competitors**, **delay-risk scores on the projects you're bidding on**, and **satellite verification of construction progress** — at $199/month.

---

## What we now offer that GTM v1 didn't

- **7 MDBs ingesting daily** (was 5): WB, IFC, ADB, AfDB, EBRD, **AIIB, IADB**.
- **20+ procurement portals automated** (national portals + MDB + UN agencies).
- **Contractor intelligence** — track which firms win bids globally, financial-distress signals on contractors in your consortia.
- **Delay prediction & early warning** — 6–9 month leading indicators across permits, contractor health, political calendars and funding gaps.
- **Satellite verification** — independent construction-progress confirmation. Ground-truth no analyst can fake.
- **Plain-English Ask** — "Active power transmission tenders in West Africa above $50M with EBRD or AfDB funding".
- **Tenders & Awards page** with AI classification + 30-day trend analytics.
- **AIIB is shipped** (was previously on roadmap).

---

## Priority targets (named, reachable)

| Name | Company | Role | Channel |
|---|---|---|---|
| Hanane Ait-Toudghi | L&T PT&D Africa | BD Director | [LinkedIn](https://www.linkedin.com/in/hanane-ait-toudghi-ing-38b051214) |
| [Vinci MENA BD] | Vinci Construction | BD Director, MENA | LinkedIn search |
| [Bechtel EMEA BD] | Bechtel | BD Director, EMEA | LinkedIn search |
| [Fluor MENA] | Fluor Corporation | BD Director | LinkedIn search |
| [CCC BD] | Consolidated Contractors | BD Manager | LinkedIn search |
| [Samsung C&T BD] | Samsung C&T | BD Director, EPC | LinkedIn search |
| [Bouygues Africa] | Bouygues Construction | Africa BD | LinkedIn search |
| [Webuild Africa] | Webuild | Africa BD | LinkedIn search |

---

## Subject lines

**Cold:** Pre-tender signals 6–18 months before RFP — for [Company]'s [region] BD desk
**Warm:** [Project Name] just moved into tender — wanted to flag

---

## Step 1 — LinkedIn DM (90 words)

```
Hi [Name],

Saw [Company] won [or bid on] [specific project / region]. I'm building InfraRadarAI: pre-tender signals from 7 MDB pipelines (WB, IFC, ADB, AfDB, EBRD, AIIB, IADB) plus 20+ procurement portals, surfaced 6–18 months before public RFP. Adds contractor intel on competitors, delay-risk scores and satellite verification of progress on active sites.

<<<<<<< HEAD
I'm building InfraRadar - a platform that surfaces pre-tender signals from World Bank, IFC, ADB, AfDB, EBRD, AIIB, and IDB project pipelines, typically 6-18 months before public RFP issuance. AI-scored projects with risk and confidence ratings, custom alerting by sector and region, and an on-demand research agent for deep project intelligence.
=======
Built for BD teams replacing the $30K MEED subscription with a live workflow at $199/mo. Pilot access (free Pro window, no card) is open.
>>>>>>> b7dd70197e259fd1d7d28ef28e353c8dee08a80e

Worth a quick call?

[Your name]
```

---

## Step 2 — Cold email (180 words)

**Subject:** Pre-tender signals for [MENA / Africa / South Asia] EPC — InfraRadarAI

```
Hi [Name],

Following up on a LinkedIn note.

<<<<<<< HEAD
I'm building InfraRadar - an AI platform that aggregates tender pipelines from the World Bank, IFC, ADB, AfDB, EBRD, AIIB, and IDB and surfaces pre-tender signals 6-18 months before RFP issuance.
=======
InfraRadarAI (infradarai.com) is what BD teams use instead of a $30K MEED subscription. Three things specific to your desk:
>>>>>>> b7dd70197e259fd1d7d28ef28e353c8dee08a80e

1. **Pre-tender signals 6–18 months before RFP** across 7 MDBs (WB, IFC, ADB, AfDB, EBRD, AIIB, IADB) and 20+ procurement portals. Today: 1,671 verified projects, $246B+ pipeline indexed, 5,657 classified alerts.

<<<<<<< HEAD
We currently track 7 MDB sources, 14 regions, 14 sectors. Coverage of IsDB, AIIB, and KfW is on the Q3 2026 roadmap.
=======
2. **Contractor intelligence** — competitor win-rate, financial-distress flags on consortia partners, sub-contractor stability. Stop discovering a partner's distress at the kick-off meeting.
>>>>>>> b7dd70197e259fd1d7d28ef28e353c8dee08a80e

3. **Delay-risk scoring** + satellite verification on projects you're bidding into — so you bid the schedule that's actually deliverable.

Plain-English Ask, custom alert rules by region/sector/value, AI-generated tender briefs (PDF). Pro is $199/month vs $30K/year for MEED. Pilot access is open: free Pro window, no card.

15-min walkthrough on a project relevant to [Company] this week or next?

[Your name]
[Your email] · infradarai.com
```

---

## Step 3 — LOI ask (after the call)

```
Hi [Name],

Thanks for the time — really useful to hear how [you currently track AfDB tenders / monitor competitor bids / chase pre-tender signals].

We're applying to [YC / Techstars / a16z Speedrun] and they want short LOIs from EPC BD professionals who've seen the platform. One paragraph, professional email, no legal commitment.

Suggested wording — edit freely:

> "I am [Name], BD [Director / Manager] at [Company]. I have reviewed InfraRadarAI (infradarai.com) and intend to trial the platform to surface pre-tender signals from MDB pipelines, monitor competitor activity, and apply contractor intelligence and delay-risk scoring across our [region/sector] portfolio. The cross-MDB aggregation, plain-English search and contractor intel are directly relevant to our BD process. I look forward to early access."

A reply with that paragraph would help us a great deal.

[Your name]
```

---

## Step 4 — Formal LOI (PDF-ready)

```
LETTER OF INTENT — INFRARADARAI EARLY ACCESS

Date: [Date]
From: [Name], [Role], [Company]
To: InfraRadarAI

I, [Name], Business Development [Director / Manager] at [Company],
express my intent to trial InfraRadarAI (infradarai.com) for our BD desk.

Capabilities directly relevant to our work:

<<<<<<< HEAD
I have reviewed InfraRadar Pulse and find the following capabilities directly relevant to our BD process:
- Pre-tender signal detection from World Bank, IFC, ADB, AfDB, EBRD, AIIB, and IDB pipelines (6-18 months before public RFP)
- AI-scored project risk and confidence assessment
- Sector and regional filtering with custom alerting
- Stakeholder intelligence extraction (contracting authority contacts, co-financiers)
- Tender calendar with upcoming bid deadline tracking
=======
1. Pre-tender signal aggregation across 7 MDB pipelines (World Bank, IFC,
   ADB, AfDB, EBRD, AIIB, IADB) and 20+ procurement portals, with 6–18
   month lead times before public RFP issuance.
>>>>>>> b7dd70197e259fd1d7d28ef28e353c8dee08a80e

2. Contractor intelligence — competitor win-rate tracking and
   financial-distress signals on consortia partners and sub-contractors.

3. Delay-risk scoring and satellite verification of construction progress
   on active sites we are bidding into or executing.

4. Plain-English search, AI-generated tender briefs, and custom alert
   rules by region, sector and value threshold.

Intended use case: [one line — e.g., "tracking AfDB and World Bank
transport tenders across West and Central Africa, with contractor
intelligence on consortium partners"].

I have no financial interest in InfraRadarAI. This letter expresses
intent to trial; it is not a binding commitment to purchase.

Signed,
[Name]
[Title]
[Company]
[Email]
[Date]
```

---

## Modules to lead with for this persona

| Module | Why it matters to EPC BD |
|---|---|
| Procurement monitoring (20+ portals) | Replaces MEED + manual portal checks |
| Pre-tender signals across 7 MDBs | 6–18 month BD lead time |
| Contractor intelligence | Competitor + consortium partner stability |
| Delay prediction & early warning | Bid schedules that are actually deliverable |
| Satellite verification | Independent ground-truth on active sites |
| Tenders & Awards page | Awards, cancellations, re-tenders, disputes |
| AI tender briefs (PDF) | Replaces consultant tender memos |
| Custom alert rules | Configure by sector × region × value × MDB |

---

## ROI talk track (lead with the math)

- Today: $30K MEED + analyst time + missed signals = real cost.
- InfraRadarAI Pro: $1,910/yr (yearly) or $199/mo. Saves $28K/year vs MEED.
- One additional pre-tender signal acted on at 3% win-rate × $50M tender × 5% margin = $75K margin upside per signal.
- Pilot access is free for the first window — risk-free to test.
